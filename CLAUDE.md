...
- **Domain, in progress (2026-09-19):** the load balancer, `dethrone-cert` and Cloud DNS zone are applied and Namecheap delegates dethroned.ai to the zone. Remaining: wait for the cert to read `ACTIVE`, then set `restrict_ingress = true` in `terraform/terraform.tfvars` and apply, which closes the *.run.app URL; after that manual builds need `_DOMAIN=dethroned.ai`. Details in `terraform/README.md`, "Domain".
