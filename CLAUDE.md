...
- `public/`: `index.html`, `style.css`, `app.js`, `og.png` (the 1200x630 share image both pages name in their `og:` tags; `scripts/og.html` is its source, rendered with headless Chrome). Client holds game state; server is stateless apart from the log. `eval.html` and `eval.js` are the chronicle at `/eval`, styled as a ledger in the same stage; every entry expands into both raw calls, and `/eval?turn=<id>` is a permalink.
...
- `docs/`: the pages for readers (`index.md`, `gameplay.md`, `turn.md`, `example-turn.md` which `scripts/example-turn-doc.mjs` generates from the real record in `docs/examples/`: regenerate it, do not hand-edit it, and refresh the record when the record shape or the prompts change, `architecture.md`, `integrity.md`, `chronicle.md`, `operations.md`), each opening with a D2 grid-snake diagram from `docs/diagrams/` (light and dark SVG, committed; `render.sh` regenerates them; `docs/diagrams/CLAUDE.md` has the rules). Numbers in the pages and diagrams (limits, constants) mirror the code and Terraform: change them together.
