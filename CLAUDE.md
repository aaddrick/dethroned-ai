...
- **Deploys:** a push to `deploy-prod` fires the Cloud Build trigger `deploy-dethrone` (connection `github-aaddrick`, repo link `aaddrick-dethroned-ai`, both in us-east4). `main` is for work; a fast-forward of `deploy-prod` is a release. Manual deploys still work with the `gcloud builds submit` command below.
